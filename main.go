...
.balanceWrap{ padding:18px; }
.balanceCard{
 background: linear-gradient(180deg, rgba(17,27,43,.95), rgba(13,20,32,.95));
 border: 1px solid rgba(0,245,212,.18);
 border-radius:26px;
 padding:22px;
 box-shadow: 0 0 35px rgba(0,245,212,.15), 0 0 60px rgba(0,245,212,.08);
}
...
.balanceValue{
 ...
 font-weight:900;
 color:white;
 text-shadow: 0 0 15px rgba(0,245,212,.6), 0 0 35px rgba(0,245,212,.35);
}
.page{ padding:18px; }
.section{
...
.btn{
 ...
 color:#04120d;
 box-shadow: 0 0 25px rgba(0,245,212,.25);
}
...
.upgradeTitle{ font-size:14px; font-weight:800; }
.upgradeDesc{
 margin-top:6px;
 color:var(--muted);
 font-size:12px;
...
<div id="generatorTab" class="section">
 <div class="coinArea">
  <svg id="coin" class="coin" viewBox="0 0 500 500">
   <defs>
   ...
   <circle cx="250" cy="250" r="220" fill="url(#coinGrad)" stroke="#d8fff4" stroke-width="8"/>
   <circle cx="250" cy="250" r="190" fill="none" stroke="rgba(255,255,255,.3)" stroke-width="4"/>
   <text x="250" y="285" text-anchor="middle" font-size="130" font-weight="900" fill="#ffffff" stroke="#004f3a" stroke-width="8">ZX</text>
  </svg>
  <div class="tapHint">Tap pentru ZX Tokens</div>
 </div>
 <div class="energyRow">
  <div class="energyBox">
  ...
  <button id="rechargeBtn" class="btn">Încarcă</button>
 </div>
 <div class="upgrades">
  <div class="upgradeCard">
   <div class="upgradeTitle">👆 Multitap Booster</div>
   <div class="upgradeDesc">Crește câștigul pe fiecare tap cu +1 ZX pentru fiecare nivel cumpărat.</div>
   <div id="tapLevel" class="upgradeMeta">Nivel 0</div>
   <div id="tapCost" class="upgradeMeta">Cost: 1000 ZX</div>
   <button id="buyTapUpgrade" class="btn" style="width:100%;margin-top:12px;">Upgrade</button>
  </div>
  <div class="upgradeCard">
   <div class="upgradeTitle">⚡ Energy Capacity Booster</div>
   <div class="upgradeDesc">Crește permanent energia maximă la 1000, 1500, 2000 și mai mult.</div>
   <div id="energyLevel" class="upgradeMeta">Max 500</div>
   <div id="energyCost" class="upgradeMeta">Cost: 2500 ZX</div>
   <button id="buyEnergyUpgrade" class="btn" style="width:100%;margin-top:12px;">Upgrade</button>
  </div>
 </div>
</div>
...
<div id="tasksTab" class="section hidden">
 ...
 <div class="upgradeCard" style="margin-bottom:14px;">
  <div class="upgradeTitle">📢 Abonare Canal Telegram</div>
  <div class="upgradeDesc">Reward instant pentru conectare.</div>
  <button id="taskTelegram" class="btn" style="width:100%;margin-top:12px;">+500 ZX</button>
 </div>
 <div class="upgradeCard" style="margin-bottom:14px;">
  <div class="upgradeTitle">🤖 Start Partener Bot</div>
  <div class="upgradeDesc">Activează partenerul și revendică recompensa.</div>
  <button id="taskPartner" class="btn" style="width:100%;margin-top:12px;">+2000 ZX</button>
 </div>
</div>

<!-- WALLET TAB -->
<div id="walletTab" class="section hidden">
 <h2 style="margin-bottom:20px; color:#d9fff5;">👛 Wallet</h2>
 <div style="background:rgba(0,0,0,.18); border-radius:20px; padding:18px; border:1px solid rgba(255,255,255,.05);">
  <div style="font-size:13px; color:#87a39d;">ZX Balance Mirror</div>
  <div id="walletBalance" style="margin-top:10px; font-size:34px; font-weight:900;">0</div>
  <button id="connectWallet" class="btn" style="margin-top:18px; width:100%;">Connect TON Wallet</button>
  <div id="walletAddress" style="display:none; margin-top:14px; color:#9fffe9; word-break:break-all; font-size:13px;"></div>
 </div>
 ...
 <h3 style="margin-top:24px; margin-bottom:14px;">Withdrawal History</h3>
 <table style="width:100%; border-collapse:collapse;">
  <thead>
   <tr>
    <th style="text-align:left; padding:12px; border-bottom:1px solid rgba(255,255,255,.08);">Date</th>
    <th style="text-align:left; padding:12px; border-bottom:1px solid rgba(255,255,255,.08);">Amount</th>
    <th style="text-align:left; padding:12px; border-bottom:1px solid rgba(255,255,255,.08);">Asset</th>
    <th style="text-align:left; padding:12px; border-bottom:1px solid rgba(255,255,255,.08);">Status</th>
   </tr>
  </thead>
...
<div id="rankTab" class="section hidden">
 ...
 <div style="margin-top:20px; background: rgba(157,77,255,.08); border: 1px solid rgba(157,77,255,.3); border-radius:20px; padding:18px;">
  <div style="font-size:13px; color:#b79cff;">Poziția ta</div>
  <div id="myRank" style="margin-top:8px; font-size:30px; font-weight:900;">#1245</div>
  <div id="myBalance" style="margin-top:8px; color:#e9dbff;">0 ZX</div>
 </div>
</div>
...
<div id="rechargeModal" style="display:none; position:fixed; inset:0; background: rgba(0,0,0,.8); backdrop-filter: blur(10px); z-index:5000; justify-content:center; align-items:center;">
 <div style="width:90%; max-width:420px; background: #101827; border-radius:24px; border: 1px solid rgba(0,245,212,.2); padding:22px;">
  ...
  <button id="closeRecharge" class="btn" style="width:100%; margin-top:12px; background:#202b3d; color:white;">Close</button>
 </div>
</div>

<div class="bottomNav">
 <div class="bottomInner">
  <button class="tabBtn active" data-tab="generator">🎮<br>Generator</button>
...
func main() {
	// Token bot (NU este recomandat în producție, dar aici e ok)
	...
	go func() {
		...
		for update := range updates {
			...
			if update.Message.Text == "/start" {
				...
				if webAppURL == "" {
					// Fallback pentru testare locală (schimbă cu URL-ul tău de Render)
					webAppURL = "https://your-render-app.onrender.com"
				}
